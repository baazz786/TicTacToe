package com.Projects.Java.TicTacToe;
...
public class ticTacToeGame {
	...
	/**
	 * This is the code for a console based TicTacToe game. You can configure
	 * the grid size of the board, by changing the value of "ROWS", "COLS"
	 * 
	 */
	...
	public static final int ROWS = 10;// To Configure the number of rows and
										// columns; Rows and columns should be
										// equal
	public static final int MIN_WIN_CRITERIA = 10;
	public static int winCriteria = 3; // Should not be greater than number of
										// Rows, if greater will be reset to
										// number of rows
	...
	// The name of the save game file to open. The file path should be same as
	// saved game
	...
	/*
	 * method to check the winning criteria Minimun should be greater than
	 * MIN_WINNING_CRITERIA Maximum is equal to the board size
	 */
	...
	public static boolean hasWon(int theSeed, int currentRow, int currentCol) {
		...
		return columnWon(theSeed, currentRow, currentCol) || rowWon(theSeed, currentRow, currentCol)
				|| diagonalWon(theSeed, currentRow, currentCol) || oppositeDiagonalWon(theSeed, currentRow, currentCol);
	...
	public static boolean diagonalWon(int theSeed, int currentRow, int currentCol) {
		int count = 0;
		boolean status = true;
		int startRow = Math.max(currentRow - (winCriteria - 1), 0);
		int startCol = Math.max(currentCol - (winCriteria - 1), 0);
		int endRow = Math.min(currentRow + (winCriteria - 1), ROWS);
		int endCol = Math.min(currentCol + (winCriteria - 1), ROWS);
		...
		} else if (endRow == (ROWS - 1) && endCol == (ROWS - 1)) {
			if (currentRow > currentCol) {
				endRow = ROWS - 1;
				endCol = currentRow;
			} else if (currentRow < currentCol) {
		...
		for (int r = startRow, c = startCol; r < endRow && c < endCol; ++r, ++c) {
			status = (board[r][c] == theSeed) && status;
			if (status) {
				++count;
				if (count == winCriteria) {
					return status;
	...
	public static void loadGame() {
		...
		try {
			...
			System.out.println("Sorry! you do not have any saved game");
		} 
